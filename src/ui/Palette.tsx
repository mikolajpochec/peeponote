...
export type ToolId = 'title' | 'text' | 'note' | 'todo' | 'link' | 'board' | 'file'
...
export const TOOLS: Tool[] = [
  ...
  { id: 'board', label: 'Board', icon: '🐸', hint: 'Nested board', w: 200, h: 140 },
...
export function Palette({ board }: { board: Board }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 left-4 flex items-center">
      <div className="pointer-events-auto flex flex-col gap-0.5 rounded-2xl border border-white/10 bg-swamp-900/90 p-1.5 shadow-2xl shadow-black/50 backdrop-blur">
        {TOOLS.map((tool) => (
          <button
            key={tool.id}
