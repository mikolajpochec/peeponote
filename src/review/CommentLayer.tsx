...
export function threadsOf(board: Board, comments: Record<string, Comment>, rects: Record<string, Record<string, { y: number; h: number }>>): Thread[] {
  ...
  return roots.map((root) => {
    ...
    return { key: anchorKey(root.anchor), root, replies, target, cardId }
  })
}
...
export function CommentLayer({ board, scale }: { board: Board; scale: number }) {
  ...
  return (
    <div className="pointer-events-none absolute left-0 top-0" data-comments>
      {/* leader lines: bubble → target */}
      {bubbles && (
        <svg className="absolute left-0 top-0 overflow-visible" width={1} height={1}>
          {threads.map((t) => {
            const p = placement.get(t.key)
            if (!p || !t.target) return null
            return <Leader key={t.key} from={p} to={t.target} muted={!!t.root.resolved} />
          })}
          {draftHere && placement.get('draft') && <Leader from={placement.get('draft')!} to={draftTarget(board, draftHere.anchor, rects)!} draft />}
        </svg>
      )}
      ...
      {[...byCard.entries()].map(([cardId, e]) => {
        ...
        return (
          <div key={cardId} className="absolute" style={{ left: card.x, top: card.y, width: card.w, height: card.h }}>
            {e.unplaced && <div className="absolute -inset-1.5 rounded-2xl border-2 border-dashed border-amber-400/80" />}
            <button
              data-nodrag
              data-badge={cardId}
              ...
              onPointerEnter={(ev) => e.unplaced && setCascade({ cardId, el: ev.currentTarget })}
              onPointerLeave={() => setCascade((c) => (c?.cardId === cardId ? null : c))}
              title={`${e.threads.length} thread${e.threads.length === 1 ? '' : 's'} — click to read`}
              className={`pointer-events-auto absolute -left-2 -top-2 flex h-6 min-w-6 items-center justify-center gap-0.5 rounded-full px-1.5 text-[11px] font-black shadow ring-2 ring-(--board-bg) ${
                e.unresolved ? (e.unplaced ? 'bg-amber-400 text-black' : 'bg-frog-500 text-white') : 'bg-swamp-600 text-frog-200'
      ...
      {threads
        ...
        .map((t) => (
          <button
            ...
            onClick={(ev) => {
              ev.stopPropagation()
              setOpenThread({ boardId: board.id, anchorKey: t.key })
            }}
            className={`pointer-events-auto absolute flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-[12px] shadow ring-2 ring-(--board-bg) ${t.root.resolved ? 'bg-swamp-600' : 'bg-frog-500 text-white'}`}
        ...
        threads.map((t) => {
          ...
          return (
            <Bubble
              ...
              onOpen={() => setOpenThread({ boardId: board.id, anchorKey: t.key })}
            />
          )
        })}
      {bubbles && draftHere && <DraftBubble board={board} place={placement.get('draft') ?? fallbackPlace(board, draftHere.anchor, rects)} onSize={(h) => report('draft', h)} />}
      ...
      {cascade && (
        <Popover anchor={cascade.el} onClose={() => setCascade(null)} sticky className="w-72 rounded-xl border border-(--hair) bg-swamp-800 p-1 shadow-2xl">
          <div className="px-2 pb-1 pt-1 text-[10px] font-black uppercase tracking-wider text-frog-200/50">No room for bubbles — {byCard.get(cascade.cardId)?.threads.length} threads</div>
          {(byCard.get(cascade.cardId)?.threads ?? []).map((t) => (
            <button
              key={t.key}
              onClick={() => setOpenThread({ boardId: board.id, anchorKey: t.key })}
...
/** thin line from the bubble's nearest edge to the target's nearest edge, dot at the target */
function Leader({ from, to, muted, draft }: { from: Rect; to: Rect; muted?: boolean; draft?: boolean }) {
  const a = nearest(from, to)
  const b = nearest(to, from)
  const color = draft ? 'rgb(251 191 36)' : muted ? 'var(--board-line)' : 'var(--board-line-sel)'
  return (
    <g opacity={muted ? 0.35 : 0.9}>
      <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={color} strokeWidth={1.5} strokeDasharray={draft ? '4 3' : undefined} />
...
const Bubble = memo(function Bubble({ thread, place, verified, onSize, onOpen }: { thread: Thread; place: Placement; verified: boolean; onSize: (h: number) => void; onOpen: () => void }) {
  ...
  return (
    <div
      ...
      className={`pointer-events-auto absolute cursor-pointer select-none rounded-lg bg-swamp-800 p-2 text-frog-50 shadow-xl ring-1 ring-(--board-line-sel) hover:ring-2 ${t.root.resolved ? 'opacity-50' : ''}`}
      style={{ left: place.x, top: place.y, width: place.w }}
    >
      <Pointer side={place.side} />
 ...
 */
function Pointer({ side, draft = false }: { side: Placement['side']; draft?: boolean }) {
  const stroke = draft ? 'rgb(251 191 36)' : 'var(--board-line-sel)'
  const sw = draft ? 2 : 1
  const fill = 'var(--color-swamp-800)'
  // the base of the wedge sits 1 px inside the bubble so it covers the ring there; only the two outer edges are stroked
  if (side === 'above')
    return (
      <svg className="absolute overflow-visible" width={16} height={9} style={{ left: 12, top: '100%', marginTop: -1 }}>
        <polygon points="0,0 8,9 16,0" fill={fill} />
        <path d="M0,0 L8,9 L16,0" fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />
      </svg>
    )
  if (side === 'below')
    return (
      <svg className="absolute overflow-visible" width={16} height={9} style={{ left: 12, bottom: '100%', marginBottom: -1 }}>
        <polygon points="0,9 8,0 16,9" fill={fill} />
        <path d="M0,9 L8,0 L16,9" fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />
      </svg>
    )
  if (side === 'right')
    return (
      <svg className="absolute overflow-visible" width={9} height={16} style={{ right: '100%', top: 10, marginRight: -1 }}>
        <polygon points="9,0 0,8 9,16" fill={fill} />
        <path d="M9,0 L0,8 L9,16" fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />
      </svg>
    )
  return (
    <svg className="absolute overflow-visible" width={9} height={16} style={{ left: '100%', top: 10, marginLeft: -1 }}>
      <polygon points="0,0 9,8 0,16" fill={fill} />
      <path d="M0,0 L9,8 L0,16" fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />
    </svg>
  )
}
...
function DraftBubble({ board, place, onSize }: { board: Board; place: Placement; onSize: (h: number) => void }) {
  ...
  return (
    <div
      ...
      className="pointer-events-auto absolute rounded-lg bg-swamp-800 p-2 text-frog-50 shadow-2xl ring-2 ring-amber-400"
      style={{ left: place.x, top: place.y, width: place.w }}
    >
      <Pointer side={place.side} draft />
